package com.qhn.mybase.utils.calendarutils.calendar;
...
import com.qhn.mybase.utils.calendarutils.Util;

import java.util.ArrayList;
import java.util.List;
import java.util.TimeZone;

import static com.qhn.mybase.utils.calendarutils.Util.checkContextNull;
...
public class CalendarProviderManager {
     ...
     * @return code:  0: success  -1: failed  -2: permission deny
     */
    public static ResultData addCalendarEvent(Context context, CalendarEvent calendarEvent) {
         /*
            TIP: 插入一个新事件的规则：
        ...
        if (null == eventUri) {
            return new ResultData(-1, null);
        }

        if (!addReminder(context, calendarEvent, eventUri, uri2,calendarEvent.getAdvanceTime()))
            return new ResultData(-1, eventUri);

        return new ResultData(0, eventUri);
    }

    /**
     * @return true 成功, false 失败
     */
    public static boolean addReminder(Context context, CalendarEvent calendarEvent, Uri eventUri, Uri uri2,int advanceTime) {
        Uri reminderUri;
